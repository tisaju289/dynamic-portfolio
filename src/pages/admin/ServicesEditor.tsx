...
const ServicesEditor = () => {
  ...
  const handleSave = async (item: any) => {
    const payload = { title: item.title, description_bn: item.description_bn, description_en: item.description_en, icon_name: item.icon_name, sort_order: item.sort_order || 0 };
    const op = item.id
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setEditing({ title: "", description_bn: "", description_en: "", icon_name: "PenTool", sort_order: services.length })} className="flex items-center gap-2 gradient-bg text-primary-foreground px-4 py-2 rounded-lg text-sm font-medium">
