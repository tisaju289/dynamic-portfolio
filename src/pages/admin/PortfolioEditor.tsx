...
const PortfolioEditor = () => {
  ...
  const handleSave = async (item: any) => {
    const payload = { title: item.title, category: item.category, description_bn: item.description_bn, description_en: item.description_en, image_url: item.image_url, link: item.link, sort_order: item.sort_order || 0 };
    const op = item.id
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setEditing({ title: "", category: "", description_bn: "", description_en: "", image_url: "", link: "", sort_order: projects.length })} className="flex items-center gap-2 gradient-bg text-primary-foreground px-4 py-2 rounded-lg text-sm font-medium">
